...
1. Toma tus fotos originales.
2. Las ajusta a un tamaño moderno y ligero (1600px).
...
def setup_logging() -> logging.Logger:
    """
    Configura el 'diario' de la aplicación.
    Aquí se anotan los errores o pasos importantes para saber qué pasó si algo falla.
    """
    logger = logging.getLogger("RedIMG")
    logger.setLevel(logging.DEBUG)
    
    if not logger.handlers:
        # Guardamos los detalles en un archivo llamado 'redimg.log'
        ...
        # También mostramos información básica en la consola
...
def get_resource_path(relative_path: str) -> str:
    """
    Esta función ayuda al programa a encontrar sus propios archivos (como el icono o imágenes).
    Es necesaria porque el programa se comporta distinto cuando se ejecuta como código 
    o cuando ya es un archivo .EXE instalado.
    """
    try:
        # Si el programa está empaquetado (como un .EXE)
        if hasattr(sys, '_MEIPASS'):
            base_path = sys._MEIPASS
        else:
            # Si estamos ejecutando el código fuente directamente
...
def get_max_threads() -> int:
    """
    Calcula cuántos trabajos puede hacer la computadora al mismo tiempo.
    Aprovecha la potencia de tu procesador para que procesar 100 fotos sea 
    mucho más rápido que hacerlo una por una.
    """
    count = os.cpu_count() or 4
    return min(count + 2, 20)   # Usamos un poco más de potencia para ganar velocidad
...
class RedIMGApp:
    """
    Esta es la clase principal que construye toda la aplicación.
    Aquí se define cómo se ve la ventana, qué pasa cuando arrastras fotos y
    cómo se guardan tus ajustes.
    """
    ...
    def load_config(self) -> None:
        """Lee tus preferencias guardadas (como el tamaño de peso elegido)."""
    ...
    def save_config(self) -> None:
        """Guarda tus preferencias para que la próxima vez que abras el programa sigan ahí."""
        config_path = "config.json"
        try:
    ...
    def corregir_orientacion(self, img: Image.Image) -> Image.Image:
        """
        Detecta si la foto fue tomada de lado o de cabeza y la endereza automáticamente.
        """
        try:
    ...
    def preparar_imagen(
        ...
        1. La endereza.
        2. La achica si es demasiado gigante (para que no pese tanto).
        3. Se asegura de que los colores se vean bien en cualquier pantalla.
        """
        if max_size is None:
            max_size = MAX_SIZE
        ...
        if max(ancho, alto) > max_size:
            ...
            # Usamos una técnica rápida para cambiar el tamaño sin perder mucha nitidez
            img = img.resize(nuevo_tam, Image.Resampling.BICUBIC)
            logger.debug("Imagen redimensionada a %dx%d", *nuevo_tam)
        
        # Si la imagen tiene transparencia (como algunos PNG), le ponemos un fondo blanco
    ...
    def optimizar_y_guardar(
        ...
        """
        Esta es la parte mágica: ajusta la calidad de la foto para que 
        quepa en el peso (KB) que elegiste.
        Hace varias pruebas rápidas hasta encontrar el punto justo entre 
        que se vea bien y que pese poco.
        """
        ...
        # Primer intento con alta calidad
        img.save(buffer_init, "JPEG", quality=92, optimize=False, progressive=False)
            
        # Si ya pesa menos de lo pedido, la guardamos de una vez
        ...
        # Si pesa mucho, empezamos a buscar la mejor calidad
        low, high = 15, 92
        final_buffer = None
        ...
        try:
            while low <= high and iterations < 4:   # Hacemos hasta 4 intentos para ser rápidos
                iterations += 1
                mid = (low + high) // 2
                buffer = BytesIO()
                
                img.save(buffer, "JPEG", quality=mid, optimize=False, progressive=False)
                
                size = buffer.tell()
                
                # Guardamos siempre la versión más pequeña por si acaso
            ...
            # Al final guardamos la mejor versión que encontramos
            ...
            # Limpiamos la memoria para que el programa no se vuelva pesado
            for buf in [final_buffer, smallest_buffer, buffer_init]:
                if buf and not buf.closed:
                    buf.close()
    
    def process_multiple_files(self, rutas: list) -> None:
        """
        Orquesta el trabajo cuando sueltas muchas fotos a la vez.
        Crea las carpetas necesarias y reparte el trabajo entre los núcleos 
        de tu computadora para terminar rápido.
        """
        try:
            total = len(rutas)
            if total == 0:
            ...
            salida_dir_name = f"{nombre_contexto}_RedIMG"
            ...
            logger.info(
                "Procesando %d imágenes con %d hilos (max_size=%d, max_weight=%dKB, formato=JPG)",
    ...
    def mostrar_about(self) -> None:
        ...
        try:
            ...
            txt = (
                "RedIMG es tu herramienta oficial para optimizar imágenes.\n"
                "Reduce el peso de tus fotos automáticamente para\n"
                "compartirlas en la web de forma rápida y sencilla."
            )
            ...
            lbl_formats_title = ttk.Label(
                self.about_window,
                text="Formatos de entrada compatibles:",
                font=("Arial", 8, "bold"),
                style="About.TLabel",
                justify="center"
            )
            lbl_formats_title.pack(pady=(12, 0), padx=10)
            ...
            lbl_formats.pack(pady=(2, 0), padx=10)
    ...
    def mostrar_config(self) -> None:
        ...
        try:
            ...
            x = main_x
            y = main_y + main_h
            self.config_window.geometry(f"300x110+{int(x)}+{int(y)}")
            ...
            lbl_size = ttk.Label(size_frame, text="Tamaño de salida:", background="#000000", foreground="#FFFFFF", font=("Arial", 10, "bold"))
            lbl_size.pack(side="left")
            
            presets = [("780KB", 780), ("1MB", 1024), ("2MB", 2048), ("Solo conversión JPG", 0)]
            ...
            # Sección de formato eliminada para RedIMG
